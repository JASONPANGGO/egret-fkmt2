    ...
    addSoundSource: function (options) {
        ...
        if (typeof options.url === 'string') {
            if (window.assetsPackage) {
                ...
                options.data = window.getAssestByKey(houzhui);
            }
...
//gameStart的时候调用播放
function playEnterSound(url, withPlay) {
    if (window.playsound === false) {
        return;
    }
    var playsound;
    ...
    if (window.mvSoundMgr.dingSound.sound) {
...
//播放音效  url:resource/assets/end.mp3
function playSoundEff(url) {
    if (window.playsound === false || soundIsMute) {
        return;
    }
